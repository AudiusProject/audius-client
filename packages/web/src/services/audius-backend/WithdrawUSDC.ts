...
const createUSDCATAInstruction = async (
  solanaUSDCATA: PublicKey,
  ...
  const createSolanaUSDCAccountInstr =
    Token.createAssociatedTokenAccountInstruction(
      ...
      solanaUSDCATA, // associatedAccount
...
const closeUSDCATAInstruction = async (
  solanaUSDCATA: PublicKey,
  solanaRootAccount: PublicKey,
  feePayer: PublicKey
) => {
  const closeSolanaUSDCAccountInstr = Token.createCloseAccountInstruction(
    TOKEN_PROGRAM_ID, //    programId
    solanaUSDCATA, //  account to close
...
const swapUSDCForSol = async (
  ...
  const transferInstructions =
    await libs().solanaWeb3Manager!.createTransferInstructionsFromCurrentUser({
      ...
      recipientSolanaAddress: solanaUSDCATA,
      instructionIndex: 1,
  ...
  const createATAInstruction = await createUSDCATAInstruction(
    solanaUSDCATA,
    solanaRootAccountPubkey,
    feePayerPubkey
  )
  const closeATAInstruction = await closeUSDCATAInstruction(
    solanaUSDCATA,
    solanaRootAccountPubkey,
    feePayerPubkey
  )
...
const getUSDCATA = async (solanaRootAccountPubkey: PublicKey) => {
...
const createAndFundDestinationATA = async (
  destinationPubkey: PublicKey,
  feePayerPubkey: PublicKey
) => {
  ...
  const solanaUSDCATA = await getUSDCATA(solanaRootAccount.publicKey)
  swapUSDCForSol(
    desiredSolAmount,
    solanaRootAccount.publicKey,
    usdcUserBank,
...
export const getOrCreateDestinationATA = async (
  ...
  if (isDestinationSolAddress) {
    ...
    // Destination ATA does not exist - create and fund it
    if (destinationAccountInfo === null) {
      await createAndFundDestinationATA(destinationPubkey, feePayerPubkey)
    }
  }
}
