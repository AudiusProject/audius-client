...
const CollectiblesPlayerContainer = ({
  ...
  const getCollectiblesArray = () => {
    let output = []
    Object.values(collectiblesInfo.ethCollectibles).forEach(colArr => output = output.concat(colArr))
    Object.values(collectiblesInfo.solCollectibles).forEach(colArr => output = output.concat(colArr))
    return output
  }
  ...
  return (
    <>
      ...
      {collectiblesInfo.type === 'detail' && (collectiblesInfo.collectible !== null) && (
        <Card
          isTwitter={isTwitter}
          backgroundColor={backgroundColor}
      ...
      {collectiblesInfo.type === 'detail' && (collectiblesInfo.collectible === null) && (
        ...
        >
          <CollectiblesHeader user={collectiblesInfo.user} />
          <p style={{ color: '#858199', textAlign: 'center' }}>Cannot find collectible.</p>
        </Card>
      )}
    </>
  )
