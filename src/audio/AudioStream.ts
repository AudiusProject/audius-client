...
class AudioStream {
  ...
  load = (
    ...
    if (forceStreamSrc) {
      console.log('making audio source or something idk lol')
      this.audio = new Audio()
      this.gainNode = null
      this.source = null
      this.audioCtx = null
      ...
      console.log('making audio source or something idk lol 2')
      this._initContext()
      if (Hls.isSupported()) {
    ...
    console.log(`DEBUG LEVEL: DURATION: ${this.duration}`)
  ...
  getPosition = () => {
    ...
    // TODO: Add logic to only run on Electron configuraion
    if (seconds !== 0) {
      //console.log(`DEBUG LEVEL: TIME CHANGE: time is set to ${seconds} 1`)
    }
    return seconds
  }
